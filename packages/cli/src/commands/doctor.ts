...
import { listWidgets } from "@aguspe/tiler-core";
...
export async function doctorCommand(opts: DoctorOptions): Promise<void> {
  ...
  out.write(
    "  " +
      kleur.dim("auth       ") +
      (flags.length === 0 ? "none (open)" : flags.join(" + ")) +
      "\n",
  );
  ...
  out.write(
    "  " +
      kleur.dim("widgets    ") +
      `${widgets.length} registered (${widgets.map((w) => w.meta.type).join(", ")})\n`,
  );

  out.write(
    "  " +
      kleur.dim("presets    ") +
      (cfg.presets.length === 0 ? "none" : cfg.presets.join(", ")) +
      "\n\n",
  );
